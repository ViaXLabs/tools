...
from csv_utils import write_check_report_csv
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Dry-run checker.\n"
            "Reads exported entities JSON, applies policy rules, outputs JSON + CSV.\n"
            "You review/edit files BEFORE applying."
        )
    )
    ...
    parser.add_argument("--out-csv", required=True, help="Output report CSV path")
    ...
    # Optional filter to only review a subset
    ...
    for ent in entities:
        ...
        if entry["action_needed"]:
            ...
            # Log what would be done (these are proposals only)
