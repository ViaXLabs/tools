...
if [[ "${MODE}" == "dry" ]]; then
  echo "== APPLY DRY-RUN =="
  if [[ -n "${FILTERS}" ]]; then
    python3 apply_newrelic_tag_changes.py --report tag_report.json --policy tag_policy.json --filters "${FILTERS}" --out-csv apply_log.csv --dry-run
  else
  ...
  if [[ -n "${FILTERS}" ]]; then
    python3 apply_newrelic_tag_changes.py --report tag_report.json --policy tag_policy.json --filters "${FILTERS}" --out-csv apply_log.csv
  else
