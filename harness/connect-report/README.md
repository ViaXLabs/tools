...
4. Same secret pattern for the other sources: `harness_api_key`,
...
- Security/quality posture (Nexus IQ, Prisma Cloud/Twistlock, Tenable.io,
  SonarQube/Checkmarx) - the `security` field in the normalized record is a
  placeholder.
- Observability (New Relic, Splunk, Prometheus/Grafana) - same for `health`.

Both slot in the same way everything else did: a `fetch/*.sh`, a Harness
secret, and a couple of lines in `normalize.py`.
