...
output "kms_key_arn" {
  description = "ARN of the customer-managed KMS key used for SSE (null when enable_customer_managed_kms=false)"
  value       = var.enable_customer_managed_kms ? aws_kms_key.firehose[0].arn : null
}
