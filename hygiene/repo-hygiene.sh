...
# If OUTFILE wasn't provided on the command-line, load it from the JSON config.
if [[ -z $OUTFILE ]]; then
  ...
  if [ "$total_missing" -gt 0 ]; then
    ...
    missing_files_joined=$(join_by " " "${missing_files[@]}")
    {
      ...
      for f in "${missing_files[@]}"; do
        ...
        dp=`dirname "$dest"`
        if [ "$dp" != "$repo" ]; then
          echo "mkdir -p \"$dp\""#!/usr/bin/env bash
# repo-hygiene.sh
#
# This script recursively scans a directory tree for Git repositories,
# compares each repository against a "model" (gold-standard) repository, and
...
#
# Dependencies:
#   - GNU Bash (compatible with Bash v3, e.g. 3.2)
#   - jq (for JSON parsing)
#
# If 'jq' is not installed, the script displays installation instructions and exits.
...
while IFS= read -r gitdir; do
  ...
  # Skip if this repository's absolute path starts with the absolute path of MODEL_REPO.
  if echo "`abspath "$repo"`" | grep -q "^`abspath "$MODEL_REPO"`"; then
    continue
  fi
